...
const App: React.FC = () => {
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    // Встраиваем API ключ напрямую (как в моей версии)
    ...
    try {
      ...
      console.log('[4/5] Отправляю запрос напрямую к Gemini API...');
  ...
  const sendCallback = async (result: any, videoUrl: string) => {
    ...
    try {
      const response = await fetch(callbackUrl, {
        ...
        // mode: 'no-cors' // Убираем no-cors для правильной обработки ответа
      });
      
      if (response.ok) {
        console.log('[ОТПРАВКА] Успешно! Результат отправлен на сервер.');
        setStatusMessage('Анализ завершен! Результат успешно отправлен на сервер.');
      } else {
        const errorText = await response.text();
        console.error(`[ОТПРАВКА] Ошибка при отправке на callback URL. Статус: ${response.status}. Ответ: ${errorText}`);
        throw new Error(`Callback-сервер ошибка: ${response.status}. ${errorText}`);
      }
      
    } catch (e: any) {
      console.error('[ОТПРАВКА] Критическая ошибка при отправке запроса. Вероятнее всего, проблема с сетью или CORS.', e);
      setStatusMessage('');
      setError(`Ошибка при отправке на callback URL. Проверьте консоль браузера (F12) на наличие сетевых проблем или ошибок CORS. Сообщение: ${e.message}`);
    }
  };
